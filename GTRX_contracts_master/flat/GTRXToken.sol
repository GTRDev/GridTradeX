...
contract Context is Initializable {
    ...
    function _msgData() internal view returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/ethereum/soGTRXity/issues/2691
        return msg.data;
    }
}
...
/**
 * @dev Wrappers over SoGTRXity's arithmetic operations with added overflow
 * checks.
 *
 * Arithmetic operations in SoGTRXity wrap on overflow. This can easily result
...
library SafeMath {
     ...
     * overflow.
     *
     * Counterpart to SoGTRXity's `+` operator.
     *
     * Requirements:
     ...
     *
     * Counterpart to SoGTRXity's `/` operator. Note: this function uses a
     * `revert` opcode (which leaves remaining gas untouched) while SoGTRXity
     * uses an invaGTRX opcode to revert (consuming all remaining gas).
     *
     * Requirements:
    ...
    function div(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
        // SoGTRXity only automatically asserts when dividing by 0
     ...
     *
     * Counterpart to SoGTRXity's `%` operator. This function uses a `revert`
     * opcode (which leaves remaining gas untouched) while SoGTRXity uses an
     * invaGTRX opcode to revert (consuming all remaining gas).
     *
     * Requirements:
     * - The divisor cannot be zero.
     */
...
interface IGTRXCertifiableToken {
...
contract GTRXStaking is Initializable, Ownable {
    ...
    modifier onlyGTRXToken {
        require(msg.sender == address(GTRXToken), "Can only be called by GTRXToken contract.");
        _;
    }
    ...
    function stake(uint amount) public whenStakingActive {
        require(amount >= 1e18, "Must stake at least one GTRX.");
        require(GTRXToken.balanceOf(msg.sender) >= amount, "Cannot stake more GTRX than you hold unstaked.");
        if (stakeValue[msg.sender] == 0) totalStakers = totalStakers.add(1);
        uint tax = _addStake(amount);
        require(GTRXToken.transferFrom(msg.sender, address(this), amount), "Stake failed due to failed transfer.");
        emit OnStake(msg.sender, amount, tax);
    }

    function unstake(uint amount) public whenStakingActive {
        require(amount >= 1e18, "Must unstake at least one GTRX.");
        require(stakeValue[msg.sender] >= amount, "Cannot unstake more GTRX than you have staked.");
        ...
        require(GTRXToken.transferFrom(address(this), msg.sender, earnings), "Unstake failed due to failed transfer.");
    ...
    function withdraw(uint amount) public whenStakingActive {
        ...
        GTRXToken.transfer(msg.sender, amount);
    ...
    function distribute(uint amount) public {
        require(GTRXToken.balanceOf(msg.sender) >= amount, "Cannot distribute more GTRX than you hold unstaked.");
        totalDistributions = totalDistributions.add(amount);
        _increaseProfitPerShare(amount);
        require(
            GTRXToken.transferFrom(msg.sender, address(this), amount),
            "Distribution failed due to failed transfer."
        );
        emit OnDistribute(msg.sender, amount);
    }

    function handleTaxDistribution(uint amount) public onlyGTRXToken {
        totalDistributions = totalDistributions.add(amount);
        _increaseProfitPerShare(amount);
        emit OnDistribute(msg.sender, amount);
    }
...
contract GTRXCertifiedPresaleTimer is Initializable, Ownable {
    using SafeMath for uint;
...
contract GTRXCertifiedPresale is Initializable, Ownable, ReentrancyGuard {
    using BasisPoints for uint;
    using SafeMath for uint;
    ...
    IGTRXCertifiableToken private token;
    IUniswapV2Router01 private uniswapRouter;
    GTRXCertifiedPresaleTimer private timer;

    mapping(address => uint) public depositAccounts;
    mapping(address => uint) public accountEarnedGTRX;
    mapping(address => uint) public accountClaimedGTRX;
    ...
    function initialize(
        ...
        address owner,
        GTRXCertifiedPresaleTimer _timer,
        IGTRXCertifiableToken _token
    ) external initializer {
    ...
    function redeem() external whenPresaleFinished {
        ...
        accountClaimedGTRX[msg.sender] = accountClaimedGTRX[msg.sender].add(claimable);
    ...
    function deposit(address payable referrer) public payable whenPresaleActive nonReentrant {
        ...
        accountEarnedGTRX[msg.sender] = accountEarnedGTRX[msg.sender].add(tokensToIssue);
    ...
    function calculateReedemable(address account) public view returns (uint) {
        ...
        uint earnedGTRX = accountEarnedGTRX[account];
        uint claimedGTRX = accountClaimedGTRX[account];
        uint cycles = finalEndTime.div(redeemInterval).add(1);
        uint totalRedeemable = earnedGTRX.mulBP(redeemBP).mul(cycles);
        uint claimable;
        if (totalRedeemable >= earnedGTRX) {
            claimable = earnedGTRX.sub(claimedGTRX);
        } else {
            claimable = totalRedeemable.sub(claimedGTRX);
        }
        return claimable;
    }
...
contract GTRXToken is
    Initializable,
    IGTRXCertifiableToken,
    ERC20Burnable,
    ERC20Mintable,
    ERC20Pausable,
    ERC20Detailed,
    ...
    GTRXStaking private GTRXStaking;
    GTRXCertifiedPresale private GTRXPresale;
    ...
    modifier onlyPresaleContract() {
        require(msg.sender == address(GTRXPresale), "Can only be called by presale sc.");
        _;
    }
    ...
    function initialize(
        ...
        address _daoFund,
        GTRXStaking _GTRXStaking,
        GTRXCertifiedPresale _GTRXPresale
    ) external initializer {
        taxBP = _taxBP;
        daoTaxBP = _daoTaxBP;
        ...
        daoFund = _daoFund;
        GTRXStaking = _GTRXStaking;
        addTrustedContract(address(_GTRXStaking));
        addTrustedContract(address(_GTRXPresale));
        setTaxExemptStatus(address(_GTRXStaking), true);
        setTaxExemptStatus(address(_GTRXPresale), true);
        //Due to issue in oz testing suite, the msg.sender might not be owner
        _transferOwnership(owner);
    }
